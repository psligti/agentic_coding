...
```python
from opencode_python.sdk import OpenCodeClient
...
```python
from opencode_python.interfaces.io import IOHandler
from opencode_python.sdk import OpenCodeAsyncClient
...
async def main() -> None:
    ...
    try:
        message = await client.add_message(session_id="invalid-id", content="Hello")
...
async def main() -> None:
    ...
    session = await with_retry(
        lambda: client.create_session("My Project"),
        max_retries=3
    )
    print(f"Created session: {session.id}")
...
**Old (Legacy):**
```python
from opencode_python.session.manager import SessionManager
