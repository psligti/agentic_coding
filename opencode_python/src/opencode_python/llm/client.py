...
class LLMClient:
    ...
    async def test_connection(self) -> Dict[str, Any]:
        ...
        if not self.api_key:
            raise ValueError(
                f"API key not configured for provider '{self.provider_id}'. "
                "Cannot test connection without API key."
            )

        test_prompt = "Test connection - respond with 'OK'"
        try:
            response_text = await self.chat_completion(
                system_prompt="You are a test assistant. Respond with 'OK' to any message.",
            ...
            logger.error(f"Connection test failed for {self.provider_id}: {e}")
            return {
                "status": "failed",
                "provider": self.provider_id,
                "model_name": self.model,
